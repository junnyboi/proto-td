...
Battlefields use an endpoint-aware isometric projection with bounded portrait panning and a shared 0.92 tactical framing multiplier. All 125 visible raised faces across S1–S16 are elevated deployment platforms for Gunner, Sniper, Mage Apprentice, Sorcerer, and Witch Doctor operators; empty raised faces never masquerade as blocked terrain. During a running battle, **Q** steps speed down through 4×, 2×, 1×, and paused, while **E** steps upward through the same bounded states; **Space** retains direct pause/resume. Recruit animation definitions share a source ground line so male and female idle/attack sprites remain centered on their tile faces in every direction. Camera, placement, economy, persistence, keyboard focus, bilingual copy, and responsive Mission Control layouts are covered by standalone Godot regressions and Xvfb visual harnesses.
...
The UI interaction suite uses generated moon-glass click, back, confirm, menu-open, and menu-close cues. The Web export captures every AudioContext before Godot boots and synchronously resumes suspended contexts on trusted pointer, touch, or keyboard input, so soundtrack and effects recover reliably from browser autoplay suspension. All runtime `AudioStreamPlayer` defaults—and the dedicated Music and SFX players explicitly—force Godot's direct streaming mixer instead of the experimental Web sample-playback graph; this keeps decoded BGM and SFX on the browser's reliable stereo output path. Battle lifecycle and every authored operator-skill semantic ID resolve to shipped audible cues rather than silently dropping playback requests. Production masters, carrier media, runtime checksums, routing, and reproduction instructions live in [`docs/audio/LUNARIS_GAMEPLAY_SCORE.md`](docs/audio/LUNARIS_GAMEPLAY_SCORE.md). Rebuild the runtime derivatives with:
...
All eleven recruit-derived specializations have regenerated adult male and female idle/attack sets in four isometric directions. The 22 resources resolve after premium portrait overrides and before legacy fallback; approved east facings and deterministic west mirrors use 640×640 source cells with a 560–640 px subject edge while Godot owns their runtime footprint. The complete visual-facing review and scoped resource-map corrections are recorded in [`docs/ADVANCED_OPERATOR_FACING_AUDIT.md`](docs/ADVANCED_OPERATOR_FACING_AUDIT.md). See [`docs/ADVANCED_OPERATOR_SPRITE_REGENERATION_V2_PLAN.md`](docs/ADVANCED_OPERATOR_SPRITE_REGENERATION_V2_PLAN.md), the historical [`docs/ADVANCED_OPERATOR_SPRITE_IMPLEMENTATION_PLAN.md`](docs/ADVANCED_OPERATOR_SPRITE_IMPLEMENTATION_PLAN.md), and `tools/operator_sprites/` for the complete GPT Image 2 → image-conditioned carrier → validated atlas pipeline and immutable provenance contract.
