...
NEW_RUNTIME_CLASSES='MusicCatalog|StageArtTheme|StageNarrative(Def|Catalog)|UiCopy|UiMaterialTier|Aetheria(Button|Label|LocaleSelector|Panel|ScreenShell|Theme)|CampaignProgression|TrainingScreen'
...
if grep -Eq 'SCRIPT ERROR: Parse Error|Failed to load script|Failed loading resource|Could not preload resource file|Could not find type|Identifier .* not declared|Identifier not found' "$tmp_root/current-boot.log"; then
...
grep -q 'title=ready staging=ready training=ready s1_squad=ready s1_results=ready s1=ready s2_children=55 s3_children=64 s2_backdrops=0 s3_backdrops=0' \
