...
func _build_body(column: VBoxContainer) -> void:
	...
	var route_header_inset := MarginContainer.new()
	route_header_inset.name = "RouteHeaderInset"
	route_header_inset.add_theme_constant_override(&"margin_left", ROUTE_HEADER_INSET)
	route_header_inset.add_theme_constant_override(&"margin_top", ROUTE_HEADER_INSET)
	route_stack.add_child(route_header_inset)
	var route_header := VBoxContainer.new()
	route_header.name = "RouteHeader"
	route_header.add_theme_constant_override(&"separation", 10)
	route_header_inset.add_child(route_header)
	...
	_build_recruitment_strip(route_header)
...
func _build_recruitment_strip(parent: VBoxContainer) -> void:
	var panel := PanelContainer.new()
	panel.name = "MissionControlRecruitDesk"
	...
	_recruitment_grid.name = "MissionControlRecruitGrid"
	_recruitment_grid.columns = 1
	_recruitment_grid.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	_recruitment_grid.add_theme_constant_override(&"h_separation", 10)
	_recruitment_grid.add_theme_constant_override(&"v_separation", 4)
	stack.add_child(_recruitment_grid)
	var summary := VBoxContainer.new()
	summary.name = "MissionControlRecruitSummary"
	summary.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	summary.add_theme_constant_override(&"separation", 2)
	_recruitment_grid.add_child(summary)
	_hire_title = AetheriaLabelType.new()
	_hire_title.name = "MissionControlRecruitTitle"
	_hire_title.apply_role(&"dense_heading")
	_hire_title.add_theme_font_size_override(&"font_size", 16)
	_hire_title.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	summary.add_child(_hire_title)
	...
	_hire_recruit.apply_compact_action_layout()
	...
	_hire_status.name = "MissionControlRecruitStatus"
	_hire_status.apply_role(&"dense_detail")
	_hire_status.add_theme_font_size_override(&"font_size", 15)
...
func _restore_hire_focus() -> void:
	if _hire_recruit != null and not _hire_recruit.disabled:
		_hire_recruit.grab_focus.call_deferred()
	elif not _enabled_rows.is_empty():
		_enabled_rows[0].grab_focus.call_deferred()
	elif _back != null:
...
func _refresh_recruitment_desk(message: String = "", error: bool = false) -> void:
	...
	_hire_status.accessibility_name = _hire_title.text
