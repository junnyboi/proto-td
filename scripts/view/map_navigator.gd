...
func relayout(stage: StageDef, viewport: Vector2) -> void:
	...
	if _is_portrait():
		...
		# Portrait stages are already rotated clockwise by BattleView. Fill from
		# terrain height up to a readable 2x cap; remaining width pans horizontally.
		scale = minf(IsoProjection.height_fill_scale(stage, viewport), PORTRAIT_MAX_SCALE)
