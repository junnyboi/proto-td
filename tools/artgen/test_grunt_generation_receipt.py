...
def refresh_local_receipt_id(item: dict[str, object]) -> None:
    ...
    basis = json.dumps(
        {
            ...
            "tool": item["tool"],
            "model": item["model"],
            "prompt_sha256": item["canonical_reproduction_prompt_sha256"],
            "references": [reference["sha256"] for reference in item["references"]],
            "output_sha256": None if output is None else output["sha256"],
            "result": item["result"],
        },
        sort_keys=True,
        separators=(",", ":"),
    )
    item["local_receipt_id"] = sha256_text(basis)
...
def mutate_direction_semantics(receipt: dict[str, object]) -> None:
    item = request(receipt, "keyframe_walk_se")
    item["canonical_reproduction_prompt"] = item["canonical_reproduction_prompt"].replace(
        "South-East front-right three-quarter isometric facing; chest lenses visible.",
        "North-East back-right three-quarter isometric facing; rear silhouette readable.",
    )
    item["canonical_reproduction_prompt_sha256"] = sha256_text(
        item["canonical_reproduction_prompt"]
    )
    refresh_local_receipt_id(item)


def mutate_false_derivation(receipt: dict[str, object]) -> None:
    receipt["compiled_source_sheets"]["walk_se"]["derivation"] = (
        "generated by an unrelated nondeterministic process"
    )
...
MUTATIONS: dict[str, Mutation] = {
    ...
    "direction_lock_semantics": mutate_direction_semantics,
    "source_derivation_exact": mutate_false_derivation,
}
