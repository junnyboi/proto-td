...
ADVANCED_COMPRESSION_FULL=1 ADVANCED_COMPRESSION_REPORT="$OUT/metrics/compression-metrics.json" \
  GODOT_SILENCE_ROOT_WARNING=1 timeout 360s godot --headless --path "$ROOT" \
...
for class_id in "${classes[@]}"; do
  xvfb-run -a -s '-screen 0 1920x1080x24' env GODOT_SILENCE_ROOT_WARNING=1 \
    godot --path "$ROOT" --audio-driver Dummy \
  ...
  for gender in female male; do
    ...
    xvfb-run -a -s '-screen 0 720x1280x24' env GODOT_SILENCE_ROOT_WARNING=1 \
      godot --path "$ROOT" --audio-driver Dummy \
        --script test/advanced_operator_live_quality_harness.gd \
