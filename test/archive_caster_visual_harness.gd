...
func _ready() -> void:
	...
	Game.default_squad = [&"caster_1"]
	var packed := load("res://scenes/battle.tscn") as PackedScene
	var battle := packed.instantiate()
	add_child(battle)
	_battle = battle
	if not battle.startup_succeeded:
		push_error("archive_caster_visual_harness: BattleView failed to start")
		return
	...
	var operator := battle._op_defs.get(&"caster_1") as OperatorDef
	if operator == null:
		push_error("archive_caster_visual_harness: caster_1 is missing")
		return
	for index: int in cells.size():
		var unit := UnitStateScript.new()
		unit.id = 300 + index
		unit.hero_id = StringName("visual-archive-caster-%d" % index)
		...
		unit.portrait_asset_id = &"portrait_archive_caster"
