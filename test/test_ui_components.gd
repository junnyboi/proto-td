...
func test_contract_fixtures_have_exact_schema_and_cardinality() -> void:
	...
	assert_eq((_inventory["states"] as Dictionary).size(), 15)
	...
	assert_eq(int(_inventory["states"]["staging_memorial_active"]["target_count"]), 7)
	assert_eq(int(_inventory["states"]["staging_memorial_active"]["text_count"]), 13)
