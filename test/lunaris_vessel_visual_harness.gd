...
func _ready() -> void:
	...
	Game.default_squad = [&"caster_2"]
	var packed := load("res://scenes/battle.tscn") as PackedScene
	var battle := packed.instantiate()
	add_child(battle)
	_battle = battle
	if not battle.startup_succeeded:
		push_error("lunaris_vessel_visual_harness: BattleView failed to start")
		return
	battle.ticks_per_frame_scale = 0.0
	battle._enemy_anim_seconds = 0.0
	_attack_mode = OS.get_environment("VESSEL_VISUAL_STATE") == "attack"
	...
	var caster := battle._op_defs.get(&"caster_2") as OperatorDef
	if caster == null:
		push_error("lunaris_vessel_visual_harness: caster_2 is missing")
		return
	for index: int in cells.size():
		var unit := UnitStateScript.new()
		unit.id = 100 + index
		unit.hero_id = StringName("visual-vessel-%d" % index)
		unit.cell = cells[index]
		unit.facing = facings[index] as UnitState.Facing
		BattleTicketRuntimeScript.copy_legacy_unit(caster, unit)
		unit.portrait_asset_id = &"portrait_lunaris_vessel"
