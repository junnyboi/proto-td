...
The action dock has a separate cause. Its generic footer routine divides all available horizontal space equally between actions and marks every button `SIZE_EXPAND_FILL`. Consequently, **Back** and the direct **Promote** action become full-width bars despite being short actions, and their role treatments do not share a consistent neutral geometry.
...
| 6. Action sizing | **Complete.** **Back** and **Promote** are 260×84, use 28-pixel labels, and enforce 24-pixel horizontal plus 12-pixel vertical internal padding in both the label and style boxes. | Neither localized action clips or overflows; both retain equal geometry and right alignment. |
