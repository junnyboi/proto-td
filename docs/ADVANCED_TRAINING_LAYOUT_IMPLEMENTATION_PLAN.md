...
| 6. Action sizing | **Complete.** **Back** and **Add to Plan** are 260×84, use 28-pixel labels, and enforce 24-pixel horizontal plus 12-pixel vertical internal padding in both the label and style boxes. | Neither localized action clips or overflows; both retain equal geometry and right alignment. |
| 7. Regression and visual contract | **Focused gates complete.** Tests cover dynamic copy, exact card/image/type scale, flat interaction styles, scrolling behavior, gutters, action padding, localization parity, Chinese rendering, UI foundations, and controller access. Xvfb captures cover 1914×797 initial/selected and 720×1280 selected states. | Reconciliation, full repository regressions, export, and the existing WebDev checkpoint remain before release completion. |
