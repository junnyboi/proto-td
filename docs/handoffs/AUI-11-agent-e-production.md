...
Python 3.12.3/Pillow 12.3.0 and Godot 4.7.1 each build every packet twice from the staged repository inputs. The cryptographically bound differential retains 133 build/differential assertions, and the 1,854-check self-regenerating repository-copy verifier rebuilds all 28 runs. Two consecutive regenerations produced stable receipt SHA-256 `2f807bd34e193815d1efc85a6020213e5fe16c8ab0394708a06d5609a83d22d0`. Fresh named gates score Vanguard role/facing 10/10, grunt role/facing 9/10, Charmed state 8/10, plus seven packet-local originality PASS results. All seven final atlas/contact pairs pass exact-candidate visual review. Attack Hit required four rejected forms before its sealed target-lozenge grammar passed; Charm VFX required three rejected forms before its four-facet-to-open-clasp progression passed. Vanguard and grunt loop failures plus every rejected state/VFX form remain external red evidence, and all final deterministic source chains reproduce from tracked contracts.
