...
**Current state:** development complete after this document’s containing commit; **integration blocked** [2]

**Shared-ledger owner:** Agent F
...
|---|---|---|
| A1 — AUI-00 durably closed on master | blocked; current repository status says `in_progress` | Stop until Agent F lands closure |
| A2 — canonical decision contains all six Round 5 hashes | blocked; current status still marks E as iterating | Agent F must record exact hashes, not prose |
| A3 — landed provenance schema represents every required fact | unverified | Map field names without dropping meaning |
| A4 — reserved probe colors remain exact | unverified against landed AUI-00 | Reconcile exact registry values; ambiguity escalates |
...
| A9 — no staging path overlap | verified during parallel development | Recheck active leases/worktrees before merge |
...
| A12 — Agent F owns shared closure ledgers | acknowledged | Agent E returns receipt only; no shared-ledger edits |
...
## 7. Required integration sequence

1. Fetch current refs and prove `AUI-00` closure is durably on master.
2. Read the canonical approval decision and compare all six hashes byte-for-byte with §3.
3. Read Agent F’s exact AUI-34 claim and reject any path overlap.
4. Run the unchanged repository gate on the claimed base.
5. Merge current master into this feature branch; resolve conflicts here only.
6. Reconcile A1–A12. Adapt mechanism or field names only; preserve every pinned semantic and failing threshold.
7. Run fresh cache-bypassed Python, Godot, and differential gates plus unchanged repository RELEASE gate on the reconciled branch.
8. Obtain a non-implementer diff-vs-plan and diff-vs-pins pass.
9. Integrate normally to master, run another fresh complete gate on the exact merged union, inspect fresh images, and prove local/remote SHA equality.
10. Agent F alone updates shared status, todo/completed ledgers, manifests, and closure records.
...
Production character and VFX generation remains deferred. `AUI-11` and later packages may use AUI-34 only after this branch lands and the runtime/provenance seam is bound by Agent F. Bulk operator production, enemies, Charm derivatives, and authoritative VFX remain outside this handoff.
