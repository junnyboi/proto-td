# TD-013 — Six-cue retro Japanese lo-fi score handoff
...
- Candidate evidence: `docs/media/TD-013-verification.json` (`mgs.verification-evidence.v2`, PASS, fresh, STANDARD)
...
- Integration renumber: Agent 2 independently integrated the Aetheria Tactics identity as TD-012 after this lane branched. Mandatory current-master reconciliation detected the collision before merge, so this music lane was renumbered to the next free stable ID, TD-013. Candidate audio commit `61eefbb` and its evidence identity are unchanged.
...
All TD-013 music, prompt, source, transcription, catalog, provenance, guide, plan, handoff, evidence, and serial-ledger reservations may be released after verified master integration. Runtime playback remains a separately scoped future seam.
