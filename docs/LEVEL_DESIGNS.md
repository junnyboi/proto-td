...
Every visible raised wall-and-top face is an elevated deployment platform. All five elevated combat definitions—Gunner, Sniper, Mage Apprentice, Sorcerer, and Witch Doctor—may occupy every such face in every stage. Historical `X` glyphs remain serialized beside `E` glyphs to preserve deterministic stage fingerprints, but both resolve through the same raised-platform placement and pointer domain; an empty raised face never rejects deployment.
