...
> **Historical technical evidence; not narrative canon.** This document records title-loop production, presentation contracts, and past verification. It does not establish narrative truth. The sole narrative authority is [`NARRATIVE_CANON.md`](NARRATIVE_CANON.md). Any older story wording visible in captures is superseded.

## Global visual definition

| Dimension | Specification |
|---|---|
| Purpose | A seamless premium title loop that extends the Lunaris resistance composition from loading into the playable title screen while preserving the established character presentation. |
...
| Field | Specification |
|---|---|
| Presentation purpose | Establish the Lunaris resistance ensemble and provide continuous ambient motion behind the title controls. |
...
The first 720×1280 pass keeps the wordmark, Start control, seed, and language selector fully visible, and pressing **Enter** advances into Company Manus command successfully. The video layer itself stretches vertically in this pass instead of preserving the 16:9 composition, so the portrait visual is not accepted. The video control must use a calculated 16:9 cover rectangle before portrait validation is repeated.

The corrected 720×1280 pass uses a centered 16:9 cover rectangle. The flagship heroine keeps natural facial and body proportions, fills the vertical hero area without letterboxing, and retains the most important reliquary geometry behind her. The faction label, wordmark, tagline, complete Start control, seed, and complete language selector remain legible; pressing **Enter** again advances into Company Manus command. Portrait title and interaction validation now pass.

## Historical Web preview verification

The browser-side managed-pack loader now reproduces the Godot loader’s gold top rule, faction/archive header, lower information band, PROTOS wordmark, staged status, percentage, moon-cyan progress bar, and detail copy. A historical 74,798,156-byte PCK request visibly reached 65% in this unified design. After the in-engine loading sequence completed, the Web canvas displayed the redesigned title with active cyan aura and constellation geometry, complete Start/language controls, and identical visual hierarchy to native desktop.
...
Pressing **Enter** on the focused Web Start control replaces the animated title with Company Manus command as intended. The console remains empty after the transition, confirming the title video shuts down with the scene and does not leak decoder or runtime errors into the campaign flow.
...
The historical final 74,800,752-byte managed PCK repeats the unified loader-to-title flow successfully in Chromium. The title-specific locale override is present in the Web export: Language, EN, and 中文 are readable, the selector remains fully contained, the cyan Start action remains dominant, and the adult character animation continues behind fixed controls.

The final browser console remains empty, and pressing **Enter** again replaces the animated title with the synchronized Company Manus command screen. Final Web runtime, decoder shutdown, focus, and scene-transition checks pass.
...
The browser console remained empty after multiple 1080p cycles. These historical PCK measurements and browser observations are retained as evidence and are not claims about the current release candidate. Pressing **Enter** replaces the title with the synchronized premium Company Manus command screen, confirming the larger Theora decoder shuts down cleanly and the Start flow remains intact.
