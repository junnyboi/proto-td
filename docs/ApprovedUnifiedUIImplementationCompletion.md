...
| Training | Roster and path actions moved into a persistent shell-level action dock outside body scrolling; selected specializations now promote their operator immediately | Rename, filters, roster selection, immediate singleton promotion, retry, safe exit, node names |
