...
| **4 — Export and deployment** | Complete / publish handoff | Synchronized runtime source `07b87523670b194ebd2aab4d6a84af284207a6ec` integrates the unified UI, 15% title readability follow-up, and soundtrack-redesign Phase 0. Direct import, bounded boot, all 19 current focused tests, English/Chinese parity, CJK glyph coverage, and error scans pass with zero failures. Twenty native landscape/portrait captures plus title-specific input captures are accepted. The Godot 4.7.2 Web export contains non-empty HTML, JavaScript, WASM, and a 98,181,684-byte PCK with checksums. Managed-preview verification proved exact resource loading, no obsolete pack requests or status, zero-chrome geometry, title containment, pointer and Enter navigation, and a clean console. Final checkpoint `085f8e53` awaits WebDev Publish-control promotion. |
