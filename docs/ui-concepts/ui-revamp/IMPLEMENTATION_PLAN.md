...
| **4 — Export and deployment** | Complete / publish handoff | Final merged runtime source `20e608d9b1d9095e67b258093dcddf2cee9e83e3` integrates the unified UI with canonical soundtrack-redesign Phase 0. Direct import, bounded boot, all 18 current focused tests, English/Chinese parity, CJK glyph coverage, and error scans pass with zero failures. Twenty native landscape/portrait captures are accepted. The Godot 4.7.2 Web export contains non-empty HTML, JavaScript, WASM, and a 98,178,176-byte PCK with checksums. Managed-preview verification proved exact resource loading, no obsolete content-pack requests or status, zero-chrome geometry, pointer and Enter navigation, and a clean console. Final checkpoint `36847d3a` awaits WebDev Publish-control promotion; the public domain still serves the prior PCK. |
