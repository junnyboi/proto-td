...
| **Annotated gameplay HUD refinement** | Complete | Live transmissions are right-aligned beneath the top-right gameplay controls with an exact 64px gap, a 12px header inset, and an 88×88 speaker visual to the left of localized speaker/body copy. The presenter resolves all authored speakers to premium, recruit, machine, or mission-poster visuals and localizes portrait accessibility names. Portraits compact to 72px below 600px width; live copy is bounded to two wrapped lines with ellipsis; the pan hint yields while a transmission is visible; and impossible short-landscape lanes suppress the noninteractive transmission rather than covering controls or deployment, restoring it when legal space returns. The landscape recruit selector doubles from its former 680px fixed maximum to 1,360px, retaining fixed 220px cards and responsive portrait reflow. The PAUSE / speed / RESIGN parent deck has 24px padding on every side. A multi-rectangle spell solver avoids active transmissions, controls, and deployment without oscillating; below 1,000px, its accessible icon-first deck compacts to three 54×50 controls while retaining tooltip names and authoritative duration/cooldown sweeps. Focused dialogue/layout/map/interaction/localization tests and clean native captures cover `1916×758`, `1280×720`, `720×1280`, `540×960`, `960×420`, and `640×360`, including Chinese portrait. |
| **Global custom-container padding and button-focus refinement** | Complete | Shared Aetheria, Lunaris, Staging, dialog, battle, Mission, Training, Premium Resonance, Results, Valhalla, cinematic-status, and warning surfaces now enforce at least 24px internal padding on every painted or textured content frame. Compact responsive branches were corrected rather than exempted: Mission cards and filters refit their true interior width, Moon Archive copy wraps inside its frame, Results and Settings stop reapplying sub-24 margins, and constrained battle Spell/transmission surfaces compact without covering gameplay. Every shared and explicit Button focus StyleBox now uses a slight translucent gold surface tint with zero border width, zero expand margins, and no cyan/white outline; pointer-hover effects remain independent. Focused regressions cover shared factories, Title/Command Center navigation, settings, Mission/Training, battle, gacha, Results, Valhalla, and transient warnings. |
| **Defeated yield padding and half-XP policy** | Complete | The Defeated Mission Yield frame now preserves exact 64px padding on all four sides in regular, compact, and portrait layouts. Campaign authority—not Results presentation—now emits survivor XP receipts by outcome: 100 XP on clear, 50 XP on ordinary leak/base defeat, and zero on voluntary resignation. V3 and legacy hash-backed resolution share one premium-exclusion filter, so non-progression premium survivors never enter either receipt path. Receipt/hash validation, reduced-motion counters, English/Chinese projection, and bottom-scrolled visual fixtures share the same values. Focused campaign/Results tests, independent review remediation, and a six-state Xvfb matrix cover the contract. |
...
| **Annotated Campaign operation-row refinement** | Complete | At the annotated `1913×766` target, the fixed Operation Route rail expands to 832px and every operation row is exactly 704px—double the former 352px row width—with exact 24px padding on all sides and a 104px text-safe minimum height. The route guidance is revised to **“Select an available operation, or replay a cleared one.”**, centered in English and Simplified Chinese. Standard landscape retains a 480px text-safe rail, portrait stays fluid and stacked, and existing hover/sparkle/selection/cinematic behavior is unchanged. Focused Campaign, localization, mission-cinematic, and shared-foundation regressions plus `1913×766`, `1280×720`, and `720×1280` native captures pass with clean logs. |
| **Cleared-operation replay selection repair** | Complete | Corrected Campaign route wiring so every unlocked operation—not only the recommended next operation—participates in pointer preview, press selection, hover feedback, and keyboard traversal. Cleared routes now select their localized **Replay available** dossier, focus an enabled **Start Mission** action, pass through the existing cinematic gate, and route the exact stage to Field Team without mutating authoritative campaign data. A reusable begin/commit/seal/resolve fixture replaces private save injection; focused replay/layout/localization tests, 85 Godot gates, the 13-test processor suite, and the standard 16-capture bilingual landscape/portrait mission matrix pass with clean diagnostics. Route labels use a local status-size override to remain single-line inside the fixed 480px rail at 150% text scale. |
