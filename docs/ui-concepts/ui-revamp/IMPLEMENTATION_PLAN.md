...
| **Annotated gameplay HUD refinement** | Complete | Live transmissions are right-aligned beneath the top-right gameplay controls with an exact 64px gap, a 12px header inset, and a 112×112 speaker visual to the left of localized speaker/body copy. The presenter resolves all authored speakers to premium, recruit, machine, or mission-poster visuals and localizes portrait accessibility names. Portraits compact to 88px below 600px width; the pan hint yields while a transmission is visible; and impossible short-landscape lanes suppress the noninteractive transmission rather than covering controls or deployment, restoring it when legal space returns. The landscape recruit selector doubles from its former 680px fixed maximum to 1,360px, retaining fixed 220px cards and responsive portrait reflow. The PAUSE / speed / RESIGN parent deck has 24px padding on every side. A multi-rectangle spell solver avoids active transmissions, controls, and deployment without oscillating. Focused dialogue/layout/map/interaction/localization tests and clean native captures cover `1916×758`, `1280×720`, `720×1280`, `540×960`, `960×420`, and `640×360`, including Chinese portrait. |
