...
These reference captures verify the title page and its settings modal at two display extremes that are not covered by the standard landscape and portrait checks. The harness forces reduced motion so the static Lunaris background is deterministic, disables title music, captures the Godot viewport directly, validates exact dimensions, and scans runtime logs for script, resource, renderer, or fatal errors.
