...
The accepted candidate passes direct Godot 4.7.2 import, bounded boot, all 39 current repository tests, localization parity, and a clean consolidated error scan. The 12-screen non-battle matrix and eight-state battle matrix pass at `1280×720` and `720×1280`. Existing authoritative inputs and mutation boundaries remain intact; visual changes are projection, layout, typography, localized copy, and focus behavior only. The last Stage Select ordering adjustment places first-clear identity above the two-line constraint block, leaving the original stage hint inside the dossier’s independent local scroller.

## Integrated Company Command recheck

The concurrent responsive Company Command architecture was merged without restoring the fabricated resource wallet or inert Messages/Settings controls. Landscape now matches the approved command-hub composition closely: a segmented identity/campaign/Exit HUD, dedicated navigation rail, character-forward center stage, and large framed mission dossier with a dominant Mission Control action. Portrait preserves the character reveal above a full-width command sheet containing mission dossier, primary action, and two-column operations grid. The segmented `UtilityPlate` now contains only the functional Exit action. The new four-viewport English/Chinese Company Command regression and the earlier authority regression both pass.
