...
- [ ] **Phase 6 — Archive:** four records, art, eight recordings, manifest, gates, imports, tests; obsolete archive media deleted. Source commit: `________` pushed to `origin/master`.
- [ ] **Phase 7 — Visual/audio direction:** loading/title and music review complete; only approved changes made; checksums/tests current. Source commit: `________` pushed to `origin/master`.
- [ ] **Phase 8 — Regression hardening:** canon/name scans, save/hash fixtures, full bilingual visual harness coverage pass. Source commit: `________` pushed to `origin/master`.
- [ ] **Phase 9 — Native final:** final reconciliation, Godot 4.7.2 import/boot, complete suite, strict logs, strict localization audit, and Xvfb pass. Final source SHA: `________________________________________` pushed.
- [ ] **Phase 10 — Web local:** fresh export hashes/sizes recorded; retired assets absent; local HTTP/browser/network/console checks pass. Source release-record commit: `________` pushed.
- [ ] **Phase 11 — Host integration:** newest host preserved; exact PCK mapped; `pnpm check`/`pnpm build`, managed preview, hash, geometry, input, locale/audio, and console checks pass. Host SHA: `________________________________________` pushed to `origin/main`.
- [ ] **Phase 12 — Public deployment:** new checkpoint `________`; public URL `________________`; public exact-resource and runtime verification pass; durable evidence pushed.
- [ ] **Final acceptance:** all fifteen criteria above reviewed and signed off; no failures or unresolved waivers remain.
