...
| **Already satisfied** | Training's transaction model is mature: validation, legal-path authority, immediate singleton atomic/idempotent dispatch, stale-state recovery, durable retry lock, and one-time acknowledgement. | Presentation refactors in [`training.gd`](../scripts/ui/training.gd) must reuse these controls/callbacks rather than reimplementing transaction logic. Successful promotion refreshes the Training roster in place. |
...
| Workstream | Production files | Tests to modify or add | Required captures |
|---|---|---|---|
| Training dock, hero dossier, and path selector | [`scripts/ui/training.gd`](../scripts/ui/training.gd), [`scripts/ui/components/promotion_path_card.gd`](../scripts/ui/components/promotion_path_card.gd), optionally a new focused selector component under `scripts/ui/components/` | Extend [`tests/campaign_ui_layout_test.gd`](../tests/campaign_ui_layout_test.gd); add `tests/training_interaction_ui_test.gd` covering rename Cancel/Confirm/error, 1/2/5 paths, immediate singleton dispatch, stale-state recovery, retry-locked exits, all origins, Mission stage/squad continuity | Replace `phase2/training-landscape.webp` and `training-portrait.webp`; add rename modal, five-path selection, immediate promotion, and retry captures in both orientations |
...
| Native validation | Godot 4.7.2 import, bounded boot, and all 39 current repository tests pass. |
