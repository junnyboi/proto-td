...
- Acceptance: a pulled worktree with the legacy `7babf28` UI registry still boots Title → Staging → Squad/Results/battles, while a registry from immediately before `CampaignProgression` loads both Training and CampaignPromotion by path; neither lane may emit a registry-dependent parse error; deleting the current CJK `.fontdata` cache never makes `aetheria_theme.gd` unparseable and the Theme still covers all exact English/Chinese catalog glyphs; source, stale-cache, and packed-project paths retain one composed font and unchanged promotion outcomes
- Required evidence: focused Theme/i18n/Training GUT; cache-bypassed font load probe; upgraded `scripts/probe_stale_class_registry.sh` with both cache snapshots and fatal-output scans; pack-only raw-font proof; one green `scripts/verify.sh` STANDARD run on the reconciled union
