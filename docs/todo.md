...
## TD-029 — Restore theme-null terrain fallback after Act I union

- Status: in_progress
- Owner: AGENT 2
- Branch: `agent-2/td-027-promote-hero-command` (TD-027 integration repair)
- Base: `origin/master` at `b2243ababaf5f0962ebb8dbc3ddfc61b499c5562`
- Dependencies: TD-027 merged-union verification
- Owned files: `docs/todo.md` TD-029 row; `docs/completed.md` TD-029 closure row; `scripts/view/iso_grid_builder.gd`
- Do not touch: map-navigation checks/thresholds; Act I theme assets/contracts; model state; promotion implementation
- Acceptance: theme-null stages return both `tile_id` and empty `cadence_id`, eliminating the fail-open dictionary access while restoring full terrain construction and existing map-navigation height/pan assertions
- Required evidence: focused `map_navigation` headless scenario and fresh merged-union `scripts/verify.sh` STANDARD PASS
