...
## TD-030 — Integrate the Company 33 Training terminal UI

- Status: blocked
- Owner: AGENT 2
- Branch: `agent-2/td-030-training-terminal-ui`
- Base: `master` at `887cc8e84aac6f8861e70d29d5bd978c21001dc1`
- Dependencies: TD-027 promotion core; P16.3 runtime authority remains external and must not be counterfeited
- Owned files: `docs/todo.md` TD-030 row; `docs/completed.md` TD-030 closure row; `FEATURES.json` P16 evidence row; `sim/campaign_state.gd`; `sim/campaign_training_projection.gd`; `autoloads/game.gd`; `scripts/ui/staging.gd`; `scenes/training.tscn`; `scripts/ui/training.gd`; `scripts/ui/components/aetheria_button.gd`; `scripts/ui/components/training_roster_row.gd`; `scripts/ui/components/promotion_path_card.gd`; `scripts/ui/components/ui_copy.gd`; `localization/en-US.json`; `test/test_training_ui.gd`; `test/test_i18n.gd`; `test/test_ui_components.gd`; `selftest/scenarios/mage_promotion.gd`; `selftest/scenarios/mage_promotion_compact.gd`; `selftest/scenarios/mage_promotion_focus.gd`; `selftest/scenarios/mage_promotion_portrait.gd`; generated `.uid` sidecars for new Godot scripts
- Do not touch: legacy/canonical campaign authority or battle projection; stage reward/recovery content cutover; save durability; model/hash/replay semantics; thresholds; unrelated UI/presentation assets
- Acceptance: Staging enables Training only for a canonical promotion-capable campaign; roster shows identity, status, XP, and plain eligibility reasons; both Mage paths show exact live data and truthful class-kit placeholders; selection and cancel are state/hash-equal; confirmation dispatches one canonical command; success preserves hero identity and projects the new duty; every rejection remains stable and localized; regular/compact/portrait layouts keep actions reachable
- Required evidence: focused GUT and i18n parity; `scripts/model_promotion_check.sh`; headless/windowed `mage_promotion`, `mage_promotion_compact`, `mage_promotion_focus`, and `mage_promotion_portrait` scenarios with completion sentinels, zero render skips, and fresh screenshots; final `scripts/verify.sh` STANDARD PASS; blocked only on Poseidon's human visual ACCEPT/REJECT verdict for the changed shipping UI
