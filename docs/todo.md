...
## TD-027 — Implement deterministic Mage Apprentice promotion command

- Status: in_progress
- Owner: AGENT 2
- Branch: `agent-2/td-027-promote-hero-command`
- Base: `master` at `8399971b7be41e17b9f05cd50768d531c27ee3d3`
- Dependencies: TD-026
- Owned files: `docs/todo.md` TD-027 row; `docs/completed.md` TD-027 closure row; `FEATURES.json` P16 evidence row; `data/campaign_def.gd`; `data/campaigns/p16_v2.tres`; `data/promotion_rules_def.gd`; `data/progression/mage_advanced_v1.tres`; `sim/campaign_state.gd`; `sim/campaign_progression.gd`; `sim/campaign_promotion.gd`; `sim/campaign_promotion_receipt_codec.gd`; `sim/campaign_promotion_proof_codec.gd`; `sim/campaign_promotion_snapshot_codec.gd`; `sim/campaign_promotion_history.gd`; `sim/campaign_save_upgrade.gd`; `sim/campaign_codec.gd`; `sim/campaign_hash.gd`; `sim/campaign_invariants.gd`; `sim/campaign_migration.gd`; `selftest/scenarios/model_roster_probe.gd`; `test/test_mage_promotion_contract.gd`; `test/test_campaign_state_p16.gd`; `test/test_hero_state.gd`; `test/test_p16_contract_fixtures.gd`; `test/test_witch_doctor.gd`; `test/fixtures/p16/campaign_v2_seed42.json`; `test/fixtures/p16/transaction_vectors_v2.json`; `tools/model_promotion_runner.gd`; `scripts/model_promotion_check.sh`; `scripts/model_roster_check.sh`; `scripts/replay_check.sh`; generated `.uid` sidecars for new Godot scripts; TD-027 handoff only if needed
- Do not touch: Training UI/localization/content cutover; battle projection runtime; `scripts/verify.sh`; thresholds; unrelated presentation/assets
- Acceptance: `promotion_options` exposes only ready 400+ XP Mage Apprentice choices; `promote_hero` permanently maps to Witch Doctor or Sorcerer with exact canonical receipt; exact retry is byte-identical; conflict, stale, invalid, and repeat requests reject with zero state change; promotion receipts round-trip through v2 save, strategic hash, resolution anchors, and canonical replay rows; two processes agree byte-for-byte
- Required evidence: focused promotion GUT suite; save/load and hash-paranoia tests; `scripts/model_promotion_check.sh`; one final `scripts/verify.sh` STANDARD run on the integrated result
