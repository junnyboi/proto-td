...
- Owned files: `autoloads/sfx.gd`; `assets/sfx/**`; `scripts/ui/deploy_bar.gd`; `scripts/view/battle_view.gd`; `test/test_sfx_player.gd*`; `selftest/scenarios/sfx_playback.gd*`; `tools/probes/stale_class_registry_boot.gd` exact locale-set remediation; `docs/decisions/D-SFX.md`; `docs/handoffs/TD-032-agent-4-sfx-integration.md`; this TD-032 row; TD-032 closure line in `docs/completed.md`; D-SFX/SFX-1 entries in `FEATURES.json`; SFX section in `CLAUDE.md`
...
- Required evidence: catalog/player GUT; `sfx_playback` and affected existing scenarios; fresh import/boot; exact stale-registry locale set on unchanged old cache; one branch STANDARD; reconciled-union gate; final master gate; hash-bound Poseidon acceptance record
