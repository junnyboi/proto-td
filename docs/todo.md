...
- Acceptance: Settings lists exactly `EN` and `中文`; selecting `zh-CN` changes the active title and every subsequent non-battle screen to a canonical Simplified Chinese catalog; the audit promotes the final hard-coded non-battle phrase (`ui.squad.tactical_hint`), and both catalogs have the same sorted 139-key set, nonblank values, and exact placeholder parity; unsupported locale or malformed/missing catalog rejects with zero locale-state change and English remains the fallback
