...
- Owned files: `assets/fonts/README.md`, `scripts/ui/components/aetheria_theme.gd`, `scripts/ui/components/training_support.gd`, `scripts/ui/components/training_support.gd.uid`, `scripts/ui/staging.gd`, `scripts/ui/training.gd`, every direct `CampaignProgression` consumer under `sim/` (`campaign_codec.gd`, `campaign_hash.gd`, `campaign_hero_codec.gd`, `campaign_invariants.gd`, `campaign_migration.gd`, `campaign_promotion.gd`, `campaign_promotion_history.gd`, `campaign_state.gd`, `campaign_strategic_commands.gd`, `campaign_training_projection.gd`, `roster_state.gd`), `scripts/probe_stale_class_registry.sh`, `tools/probes/stale_class_registry_boot.gd`, `tools/probes/promotion_cache_boot.gd`, `tools/probes/promotion_cache_boot.gd.uid`, `test/test_ui_components.gd`, `test/test_ui_components.gd.component-contract.json`, `test/test_training_ui.gd`, `docs/todo.md`, `docs/completed.md`, `CLAUDE.md`
...
- Acceptance: a pulled worktree with the legacy `7babf28` UI registry still boots Title → Staging → Squad/Results/battles, while the current registry with only the new `CampaignProgression` entry removed loads both Training and CampaignPromotion by path; neither lane may emit a registry-dependent parse error; deleting the current CJK `.fontdata` cache never makes `aetheria_theme.gd` unparseable and the Theme still covers all exact English/Chinese catalog glyphs; source, stale-cache, and packed-project paths retain one composed font and unchanged promotion outcomes
- Required evidence: focused Theme/i18n/Training GUT; cache-bypassed font load probe; upgraded `scripts/probe_stale_class_registry.sh` with legacy UI plus current-minus-progression snapshots and fatal-output scans; pack-only raw-font proof; one green `scripts/verify.sh` STANDARD run on the reconciled union
