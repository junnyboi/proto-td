...
- Status: in_progress
- Owner: AGENT 11
- Branch: `agent-11/zh-cn-localization`
- Base: `master` at `ef740d273034df4147fc004fc6246ebb82ec7641`
- Dependencies: AUI-12 stable-key localization foundation
- Owned files: `autoloads/i18n.gd`, `localization/en-US.json`, `localization/zh-CN.json`, `assets/fonts/ProtosSansSC-Subset.otf`, `assets/fonts/NotoSansCJK-COPYRIGHT.txt`, `assets/fonts/README.md`, `scenes/ui/components/aetheria_locale_selector.tscn`, `scripts/ui/components/aetheria_theme.gd`, `scripts/ui/components/aetheria_button.gd` (fan-in metric fix), `scripts/ui/components/aetheria_locale_selector.gd`, `scripts/ui/components/ui_copy.gd`, `scripts/ui/title.gd`, `scripts/ui/staging.gd`, `scripts/ui/squad_select.gd`, `test/test_i18n.gd`, `test/test_ui_components.gd`, `test/test_ui_components.gd.component-contract.json`, `test/test_ui_registry_bootstrap.gd`, `selftest/scenarios/ui_shell_floor.gd`, `selftest/scenarios/ui_shell_floor.gd.contract.json`, `docs/todo.md`, `docs/completed.md`
- Do not touch: simulation, save/hash/replay, gameplay resources, battle HUD, `scripts/verify.sh`, thresholds, or unrelated presentation files
- Acceptance: Settings lists exactly `EN` and `中文`; selecting `zh-CN` changes the active title and every subsequent non-battle screen to a canonical Simplified Chinese catalog; the audit promotes the final hard-coded non-battle phrase (`ui.squad.tactical_hint`) and all seven compact action labels introduced during fan-in; both catalogs have the same sorted 146-key set, nonblank values, and exact placeholder parity; unsupported locale or malformed/missing catalog rejects with zero locale-state change and English remains the fallback
- Required evidence: focused localization/UI GUT, Chinese glyph coverage, English↔Chinese runtime switch assertions, `ui_shell_floor` headless/windowed with fresh Chinese Title/Staging/Campaign/Squad/Results screenshots, and one green `scripts/verify.sh` STANDARD run on the reconciled union
