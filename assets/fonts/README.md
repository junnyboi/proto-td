...
At runtime, `AetheriaTheme` loads the tracked OTF source through `FontFile.load_dynamic_font()` before consulting Godot's imported-resource loader. This keeps UI scripts compilable when `.godot/imported/*.fontdata` is missing or stale. Exported packs retain a `ResourceLoader` fallback for environments where the original source file is not directly accessible. The generated `.fontdata` remains a disposable cache, never a source artifact.
