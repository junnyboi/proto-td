...
const REQUIRED_SHOTS: Array[String] = [
	"canon_staging_1280x720",
	"canon_staging_720x1280",
	"canon_s1_squad_1280x720",
	"canon_s1_squad_720x1280",
	"canon_s6_squad_1280x720",
	"canon_s8_squad_1280x720",
	"canon_s1_clear_1280x720",
	"canon_s6_clear_1280x720",
	"canon_s6_defeat_1280x720",
	"canon_s7_clear_1280x720",
	"canon_s8_clear_1280x720",
	"canon_missing_record_1280x720",
]
...
func run(h: SelfTestHarness) -> void:
	...
	h.check(
		"canon contract exact shot basenames", contract.get("shot_basenames", []) == REQUIRED_SHOTS
	)
	h.check(
		"canon catalog valid",
		(NARRATIVE_CATALOG as StageNarrativeCatalogType).validate_contract().is_empty()
	)
	...
	h.check(
		"canon scenario requested exact twelve-shot basename set",
		requested_sorted == required_sorted and _requested_shots.size() == 12,
		str(_requested_shots)
	)
	if ok:
		h.done()
...
func _ordinary_s1(h: SelfTestHarness, game: Node) -> bool:
	...
	if staging == null:
		return false
	if not await _open_s1_squad(h, game, staging):
		return false
	return await _clear_real_s1(h, game)


func _open_s1_squad(h: SelfTestHarness, game: Node, staging: Control) -> bool:
	...
	h.root.size = Vector2i(1280, 720)
	await h.frames(4)
	return await _pick_and_launch_s1(h, game, squad)


func _pick_and_launch_s1(h: SelfTestHarness, game: Node, squad: Control) -> bool:
	var squad_scroll := _find(squad, "SquadScroll") as ScrollContainer
	var ready: Array = game.call("campaign_projection")["ready_heroes"]
	for hero: Dictionary in ready.slice(0, 3):
		var hero_id := StringName(hero["hero_id"])
		var pick := _find(squad, "Pick_%s" % hero_id) as Button
		if not await _ensure_scroll_control_visible(
			h,
			squad_scroll,
			pick,
			"ordinary S1 Recruit card",
		):
			return false
...
func _clear_real_s1(h: SelfTestHarness, game: Node) -> bool:
	...
	while model.result == BattleModel.Result.RUNNING and model.tick < 4000:
		while index < timeline.size() and int(timeline[index][0]) == model.tick:
			h.check(
				"real S1 scripted action accepted",
				model.apply_action((timeline[index] as Array).slice(1))
			)
			index += 1
		model.step()
		if model.tick % 300 == 0:
...
func _missing_record_projection(h: SelfTestHarness, game: Node) -> bool:
	...
	h.check(
		"missing record renders exact fallback",
		(
			objective != null
			and objective.text.contains("Mission record unavailable. Return to Mission Control.")
		)
	)
	h.root.size = Vector2i(1280, 720)
	await h.frames(5)
...
func _inspect_staging(
	h: SelfTestHarness, staging: Control, viewport: Vector2i, shot: String
) -> void:
	...
	h.check(
		"Company 33 visible", heading != null and heading.text.to_lower().contains("company 33")
	)
	h.check(
		"Staging canon terms visible",
		(
			body != null
			and body.text.contains("Hearthcross")
			and body.text.contains("Great Flare")
			and body.text.contains("Custodian")
		)
	)
	h.check("Staging S1 next title", title != null and title.text == "NEXT 1: First Stand")
	h.check(
		"Staging dense heading exact",
		heading != null and heading.get_theme_font_size(&"font_size") == 34
	)
	h.check(
		"Staging dense body exact", body != null and body.get_theme_font_size(&"font_size") == 34
	)
	h.check(
		"Staging next title exact", title != null and title.get_theme_font_size(&"font_size") == 34
	)
	h.check(
		"Staging objective exact",
		objective != null and objective.get_theme_font_size(&"font_size") == 34
	)
...
func _check_squad_semantics(h: SelfTestHarness, squad: Control, stage_id: StringName) -> void:
	var record: StageNarrativeDefType = (NARRATIVE_CATALOG as StageNarrativeCatalogType).get_record(
		stage_id
	)
	...
	for specification: Array in [
		...
		h.check(
			"%s %s full copy" % [stage_id, specification[0]],
			label != null and label.text.ends_with(expected),
			label.text if label != null else "missing"
		)
		h.check(
			"%s %s dense font exact" % [stage_id, specification[0]],
			label != null and label.get_theme_font_size(&"font_size") == 34
		)
	var hint := _find(squad, "TacticalHint") as Label
	var stage := load("res://data/stages/%s.tres" % stage_id) as StageDef
	h.check(
		"%s full tactical hint" % stage_id,
		hint != null and hint.text.ends_with(UiCopyType.stage_hint(stage))
	)
	h.check(
		"%s tactical hint dense font exact" % stage_id,
		hint != null and hint.get_theme_font_size(&"font_size") == 34
	)


func _inspect_squad_geometry(
	h: SelfTestHarness, squad: Control, viewport: Vector2i, shot: String
) -> void:
	h.root.size = viewport
	await h.frames(5)
	...
	h.check(
		"%s squad heading dense font exact" % shot,
		heading != null and heading.get_theme_font_size(&"font_size") == 34
	)
	for node_name: String in [
		"BriefingObjective", "BriefingThreat", "BriefingHumanReason", "BriefingClue", "TacticalHint"
	]:
		...
		if viewport.x >= viewport.y and node != null and grid != null:
			h.check(
				"%s does not overlap roster" % node_name,
				not node.get_global_rect().intersects(grid.get_global_rect())
			)
	if start != null and grid != null:
		h.check(
			"primary action does not overlap roster",
			not start.get_global_rect().intersects(grid.get_global_rect())
		)
	await _shot(h, shot, viewport)


func _check_result_semantics(
	h: SelfTestHarness, results: Control, stage_id: StringName, clear: bool
) -> void:
	var record: StageNarrativeDefType = (NARRATIVE_CATALOG as StageNarrativeCatalogType).get_record(
		stage_id
	)
	...
	h.check(
		"%s exact %s consequence" % [stage_id, "clear" if clear else "defeat"],
		consequence != null and consequence.text == UiCopyType.stage_narrative_text(record, field),
		consequence.text if consequence != null else "missing"
	)
	h.check(
		"%s consequence dense font exact" % stage_id,
		consequence != null and consequence.get_theme_font_size(&"font_size") == 34
	)
	for action_name: String in ["RetryButton", "ReturnToStaging", "BackToTitle"]:
		var action := _find(results, action_name) as Button
		h.check(
			"post-battle %s remains enabled" % action_name, action != null and not action.disabled
		)
...
func _inspect_results_geometry(h: SelfTestHarness, results: Control, shot: String) -> void:
	...
	h.check(
		"%s consequence heading dense font exact" % shot,
		heading != null and heading.get_theme_font_size(&"font_size") == 34
	)
	...
	if line != null and actions != null:
		h.check(
			"%s consequence does not overlap actions" % shot,
			not line.get_global_rect().intersects(actions.get_global_rect())
		)
...
func _result_shot(stage_id: StringName, result_kind: int) -> String:
	if stage_id == &"s6":
		return (
			"canon_s6_clear_1280x720"
			if result_kind == BattleModel.Result.CLEAR
			else "canon_s6_defeat_1280x720"
		)
...
func _check_rect(h: SelfTestHarness, node: Control, owner: Control, label: String) -> void:
	...
	h.check(
		"%s nonzero visible" % label,
		node.is_visible_in_tree() and rect.size.x > 0.0 and rect.size.y > 0.0,
		str(rect)
	)
	if owner != null:
		var in_scroll_content := owner is ScrollContainer and owner.is_ancestor_of(node)
		h.check(
			"%s inside %s" % [label, owner.name],
			in_scroll_content or owner.get_global_rect().intersects(rect),
			"%s in %s" % [rect, owner.get_global_rect()]
		)
...
func _save_game(game: Node) -> Dictionary:
	return {
		...
		"campaign_store": game.get("campaign_store"),
		...
		"last_campaign_error": game.get("last_campaign_error"),
		"debug_override": game.get("_debug_catalog_override"),
		"campaign_context": (game.get("_campaign_context") as Dictionary).duplicate(true),
		"pending_ticket": (game.get("_pending_battle_ticket") as Dictionary).duplicate(true),
		"pending_mutation": game.get("_pending_campaign_mutation"),
		"campaign_battle_active": game.get("_campaign_battle_active"),
	}
...
func _restore_game(game: Node, saved: Dictionary) -> void:
	...
	game.set("campaign_store", saved["campaign_store"])
	...
	game.set("last_campaign_error", saved["last_campaign_error"])
	game.set("_debug_catalog_override", saved["debug_override"])
	game.set("_campaign_context", saved["campaign_context"])
	game.set("_pending_battle_ticket", saved["pending_ticket"])
	game.set("_pending_campaign_mutation", saved["pending_mutation"])
	game.set("_campaign_battle_active", saved["campaign_battle_active"])
...
func _await_screen(h: SelfTestHarness, game: Node, marker: String) -> Control:
	...
	while budget > 0:
		...
		if (
			...
			and _find(content, marker) != null
		):
			var candidate := content as Control
			await h.frames(3)
			if (
				is_instance_valid(candidate)
				and game.get("content") == candidate
				and _find(candidate, marker) != null
			):
				return candidate
		budget -= 1
		await h.frames(1)
