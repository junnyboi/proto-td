...
func _test_argument_contract() -> void:
	...
	for invalid: String in [
		...
		"--content-pack=enemy-variants|ftp://cdn.example/a.zip|1234|%s" % digest,
		"--content-pack=enemy-variants|https://cdn.example/a.zip|0|%s" % digest,
		"--content-pack=enemy-variants|https://cdn.example/a.zip|1234|abc",
	]:
		_check(LoaderType.parse_argument(invalid).is_empty(), "invalid content-pack argument was accepted")
	_check(LoaderType.valid_pack_ids().size() == 12, "expected exact 12 admitted content pack ids")


func _test_resource_routing() -> void:
	_check(
		LoaderType.pack_id_for_resource("res://assets/enemy-variants/breacher_attack_ne.webp") == "enemy-variants",
		"enemy variant path did not route to its pack",
	)
	_check(
...
func _test_autoload_contract() -> void:
	...
	loader.call("configure", PackedStringArray([
		"--content-pack=enemy-variants|https://cdn.example/enemy.zip|2048|%s" % digest,
