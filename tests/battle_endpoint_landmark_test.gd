...
func _run() -> void:
	...
	_validate_manifest(&"world.act1.spawn", Vector2i(64, 64), 24, 12.0)
	_validate_manifest(&"world.act1.core", Vector2i(64, 80), 6, 6.0)
	...
	_check(portal.size == Vector2(64, 64), "portal frame exceeds one-tile runtime box")
...
func _validate_manifest(id: StringName, expected_size: Vector2i, frames: int, fps: float) -> void:
	_check(Art.size(id) == expected_size, "%s frame size drifted" % id)
