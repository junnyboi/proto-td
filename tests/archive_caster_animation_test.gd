...
const IDLE_IDS := {
	&"ne": &"op_anim_archive_caster_idle_ne",
	&"nw": &"op_anim_archive_caster_idle_nw",
	&"se": &"op_anim_archive_caster_idle_se",
	&"sw": &"op_anim_archive_caster_idle_sw",
}
const ATTACK_IDS := {
	&"ne": &"op_anim_archive_caster_attack_ne",
	&"nw": &"op_anim_archive_caster_attack_nw",
	&"se": &"op_anim_archive_caster_attack_se",
	&"sw": &"op_anim_archive_caster_attack_sw",
}
...
func _init() -> void:
	...
	if _failures.is_empty():
		print("ARCHIVE_CASTER_ANIMATION_TEST_OK")
		quit(0)
		return
...
func _test_premium_identity_routing() -> void:
	...
	premium.op_id = &"caster_1"
	premium.class_id = &"mage_apprentice"
	premium.portrait_asset_id = &"portrait_archive_caster"
	_check(
		OperatorVisualCatalogScript.template_for_unit(
			premium.op_id, premium.portrait_asset_id, premium.hero_id, premium.id, premium.class_id,
		) == &"archive_caster",
		"Archive Caster portrait did not resolve to the unique visual template",
	)
	var ordinary := UnitStateScript.new()
	ordinary.op_id = &"caster_1"
	ordinary.class_id = &"mage_apprentice"
	ordinary.portrait_asset_id = &"portrait_caster_1"
	...
	_check(
		ordinary_template in [&"mage_apprentice_female", &"mage_apprentice_male"],
		"ordinary caster_1 did not resolve to a generated Mage Apprentice variant",
	)


func _test_catalog_contract() -> void:
	var animation := OperatorVisualCatalogScript.get_animation(&"archive_caster")
	_check(animation != null, "Archive Caster animation definition is missing")
	if animation == null:
		return
	_check(animation.visual_id == &"operator_archive_caster", "visual_id drifted")
	...
	_check(not animation.placeholder, "Archive Caster animation is still marked placeholder")
...
func _compare_mirror(east_name: String, west_name: String, frame_count: int) -> void:
	var root := "res://assets/sprites/operators/animated/archive_caster/"
