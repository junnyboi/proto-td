		...
		for child: Node in operator_grid.get_children():
			if child is Button:
				...
				if portrait_node != null:
					...
					_check(is_equal_approx(portrait_node.offset_left, portrait_edge_inset - 112.0) and is_equal_approx(portrait_node.offset_right, -portrait_edge_inset - 112.0), "%s operator portrait is not shifted left by exactly 112px total" % label)
					_check(card_label == null or card_label.get_global_rect().end.x <= portrait_node.get_global_rect().position.x + 112.0 + EPSILON, "%s operator information extends past the portrait pane's original boundary" % label)
