...
func _run() -> void:
	...
	_check(dock_cancel.custom_minimum_size.is_equal_approx(Vector2(280, 92)), "Cancel action is not a standard compact size")
	_check(confirm_pull.custom_minimum_size.is_equal_approx(Vector2(280, 92)), "Resonate action is not a standard compact size")
	_check(confirm_actions.size_flags_horizontal == Control.SIZE_SHRINK_END, "confirmation actions are not right aligned")
	_check(confirm_dock.get_global_rect().end.x - confirm_actions.get_global_rect().end.x <= 20.0, "narrow actions do not align to the dock's right edge")
	var cancel_style := dock_cancel.get_theme_stylebox(&"normal")
	var confirm_style := confirm_pull.get_theme_stylebox(&"normal")
	_check(cancel_style is StyleBoxFlat and confirm_style is StyleBoxFlat, "confirmation actions still use ornate strike-through textures")
	_check(cancel_style.content_margin_left >= 32.0 and cancel_style.content_margin_top >= 18.0, "Cancel action padding is claustrophobic")
	_check(confirm_style.content_margin_left >= 32.0 and confirm_style.content_margin_top >= 18.0, "Resonate action padding is claustrophobic")
	...
	_check(confirm_actions.size.x <= 576.0, "wide confirmation actions stretched beyond standard size")
	_check(confirm_dock.get_global_rect().end.x - confirm_actions.get_global_rect().end.x <= 20.0, "wide confirmation actions are not right aligned")
