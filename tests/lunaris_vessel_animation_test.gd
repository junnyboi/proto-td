...
const IDLE_IDS := {
	&"ne": &"op_anim_lunaris_vessel_idle_ne",
	&"nw": &"op_anim_lunaris_vessel_idle_nw",
	&"se": &"op_anim_lunaris_vessel_idle_se",
	&"sw": &"op_anim_lunaris_vessel_idle_sw",
}
const ATTACK_IDS := {
	&"ne": &"op_anim_lunaris_vessel_attack_ne",
	&"nw": &"op_anim_lunaris_vessel_attack_nw",
	&"se": &"op_anim_lunaris_vessel_attack_se",
	&"sw": &"op_anim_lunaris_vessel_attack_sw",
}
...
func _init() -> void:
	...
	if _failures.is_empty():
		print("LUNARIS_VESSEL_ANIMATION_TEST_OK")
		quit(0)
		return
...
func _test_catalog_contract() -> void:
	var animation := OperatorVisualCatalogScript.get_animation(&"lunaris_vessel")
	_check(animation != null, "Vessel animation definition is missing")
	if animation == null:
		return
	_check(animation.visual_id == &"operator_lunaris_vessel", "visual_id drifted")
	...
	_check(not animation.placeholder, "Vessel animation is still marked placeholder")
...
func _test_exact_mirrors() -> void:
	_compare_mirror(
		"res://assets/sprites/operators/animated/lunaris_vessel/idle_ne.webp",
		"res://assets/sprites/operators/animated/lunaris_vessel/idle_nw.webp",
		24,
	)
	_compare_mirror(
		"res://assets/sprites/operators/animated/lunaris_vessel/idle_se.webp",
		"res://assets/sprites/operators/animated/lunaris_vessel/idle_sw.webp",
		24,
	)
	_compare_mirror(
		"res://assets/sprites/operators/animated/lunaris_vessel/attack_ne.webp",
		"res://assets/sprites/operators/animated/lunaris_vessel/attack_nw.webp",
		13,
	)
	_compare_mirror(
		"res://assets/sprites/operators/animated/lunaris_vessel/attack_se.webp",
		"res://assets/sprites/operators/animated/lunaris_vessel/attack_sw.webp",
		13,
	)
	_check(
...
func _compare_mirror(east_path: String, west_path: String, frame_count: int) -> void:
	var east := Image.load_from_file(ProjectSettings.globalize_path(east_path))
	var west := Image.load_from_file(ProjectSettings.globalize_path(west_path))
	_check(east != null and not east.is_empty(), "%s failed to load" % east_path)
	_check(west != null and not west.is_empty(), "%s failed to load" % west_path)
	if east == null or west == null or east.is_empty() or west.is_empty():
		return
	_check(east.get_size() == Vector2i(192 * frame_count, 192), "%s strip size mismatch" % east_path)
	_check(west.get_size() == east.get_size(), "%s strip size mismatch" % west_path)
	for frame: int in frame_count:
		...
		if east_frame.get_data() != west_frame.get_data():
			_failures.append("mirror mismatch: %s frame %d" % [west_path, frame])
			return
