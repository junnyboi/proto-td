...
func _run_source_contract() -> void:
	...
	if prefetch != null:
		...
		prefetch.call("set_background_download_policy", true, 1)
		prefetch.call("prefetch_from_title", Vector2(1280, 720), all_arguments)
		_check(prefetch.call("queued_stream_keys").size() == 1, "slow-network policy did not cap Resonance prefetch at one")
		prefetch.call("set_background_download_policy", false, 0)
		_check(prefetch.call("queued_stream_keys").is_empty(), "metered preference did not clear Resonance background work")
		_check(not bool(prefetch.call("background_downloads_enabled")), "Resonance prefetch did not retain the disabled policy")
		prefetch.call("reset_for_tests")
		prefetch.call("configure_streams", all_arguments)
