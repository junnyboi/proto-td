...
func _run_source_contract() -> void:
	...
	_check(finished_count[0] == 1, "natural playback completion did not emit cinematic_finished")
	_check(not player.video_player().is_playing(), "natural playback completion did not stop video")
	_check(not player.video_player().visible, "natural playback completion left video visible")
	_check(player.final_plate().visible, "natural playback completion did not expose final plate")
	player.video_player().finished.emit()
